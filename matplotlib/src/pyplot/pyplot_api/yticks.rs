    ...
    pub fn set(self) -> PyResult<()> {
        Python::attach(|py| {
            ...
            if let Some(labels_vec) = self.labels {
                ...
                ax.call_method("set_yticks", (ticks_list, labels_list), Some(&kwargs))?;
            } else {
                ax.call_method("set_yticks", (ticks_list,), Some(&kwargs))?;
            }
